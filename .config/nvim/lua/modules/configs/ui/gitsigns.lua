    ...
    on_attach = function(bufnr)
      ...
      bind.nvim_load_mapping({
        ["n|]g"] = bind.map_callback(function()
          ...
          return "<Ignore>"
        end):with_buffer(bufnr):with_expr():with_desc("git: Goto next hunk"),
        ["n|[g"] = bind.map_callback(function()
          if vim.wo.diff then
            return "[g"
          end
          ...
          return "<Ignore>"
        end):with_buffer(bufnr):with_expr():with_desc("git: Goto prev hunk"),
        ["n|<leader>hs"] = bind.map_callback(function()
          require("gitsigns.actions").stage_hunk()
        end):with_buffer(bufnr):with_desc("git: Stage hunk"),
        ...
        end):with_buffer(bufnr):with_desc("git: Reset hunk"),
