...
local function handler(key, info, mode)
  ...
  elseif info.close then
    if info.escape and key ~= '<Space>' then
      local check_line, check_col
      if mode == 'insert' then
        check_line = vim.api.nvim_get_current_line()
        check_col = vim.api.nvim_win_get_cursor(0)[2]
      else
        check_line = vim.fn.getcmdline()
        check_col = vim.fn.getcmdpos() - 1
      end
      local left_text = check_line:sub(1, check_col)
      local _, count = left_text:gsub(vim.pesc(key), '')
      if count % 2 == 1 then
        return key
      end
    end
